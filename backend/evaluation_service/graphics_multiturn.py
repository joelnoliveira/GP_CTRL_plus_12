...
# codellama
with open(
    f"results/attacker_llm/{malicious_or_vulnerable}/multi_turn/codellama_1.json", "r"
) as f:
    data_codellama_1 = json.load(f)

with open(
    f"results/attacker_llm/{malicious_or_vulnerable}/multi_turn/codellama_2.json", "r"
) as f:
    data_codellama_2 = json.load(f)

with open(
    f"results/attacker_llm/{malicious_or_vulnerable}/multi_turn/codellama_3.json", "r"
) as f:
    data_codellama_3 = json.load(f)

# llama2
with open(
    f"results/attacker_llm/{malicious_or_vulnerable}/multi_turn/llama2_1.json", "r"
) as f:
    data_llama2_1 = json.load(f)

with open(
    f"results/attacker_llm/{malicious_or_vulnerable}/multi_turn/llama2_2.json", "r"
) as f:
    data_llama2_2 = json.load(f)

with open(
    f"results/attacker_llm/{malicious_or_vulnerable}/multi_turn/llama2_3.json", "r"
) as f:
    data_llama2_3 = json.load(f)

# zephyr
with open(
    f"results/attacker_llm/{malicious_or_vulnerable}/multi_turn/zephyr_1.json", "r"
) as f:
    data_zephyr_1 = json.load(f)

with open(
    f"results/attacker_llm/{malicious_or_vulnerable}/multi_turn/zephyr_2.json", "r"
) as f:
    data_zephyr_2 = json.load(f)

with open(
    f"results/attacker_llm/{malicious_or_vulnerable}/multi_turn/zephyr_3.json", "r"
) as f:
    data_zephyr_3 = json.load(f)

# llama3.1
with open(
    f"results/attacker_llm/{malicious_or_vulnerable}/multi_turn/llama3.1_8b_1.json", "r"
) as f:
    data_llama3_1_8b_1 = json.load(f)

with open(
    f"results/attacker_llm/{malicious_or_vulnerable}/multi_turn/llama3.1_8b_2.json", "r"
) as f:
    data_llama3_1_8b_2 = json.load(f)

with open(
    f"results/attacker_llm/{malicious_or_vulnerable}/multi_turn/llama3.1_8b_3.json", "r"
) as f:
...
if malicious_or_vulnerable == "malicious":
    # llama3.1:70b
    with open(
        f"results/attacker_llm/{malicious_or_vulnerable}/multi_turn/llama3.1_70b_1.json",
        "r",
    ) as f:
        data_llama3_1_70b_1 = json.load(f)

    with open(
        f"results/attacker_llm/{malicious_or_vulnerable}/multi_turn/llama3.1_70b_2.json",
        "r",
    ) as f:
        data_llama3_1_70b_2 = json.load(f)

    with open(
        f"results/attacker_llm/{malicious_or_vulnerable}/multi_turn/llama3.1_70b_3.json",
        "r",
    ) as f:
    ...
    # gemma3:27b
    with open(
        f"results/attacker_llm/{malicious_or_vulnerable}/multi_turn/gemma3_27b_1.json",
        "r",
    ) as f:
        data_gemma3_27b_1 = json.load(f)

    with open(
        f"results/attacker_llm/{malicious_or_vulnerable}/multi_turn/gemma3_27b_2.json",
        "r",
    ) as f:
        data_gemma3_27b_2 = json.load(f)

    with open(
        f"results/attacker_llm/{malicious_or_vulnerable}/multi_turn/gemma3_27b_3.json",
        "r",
    ) as f:
...
assert (
    len(data_codellama_1)
    == len(data_codellama_2)
    == len(data_codellama_3)
    == len(data_llama2_1)
    == len(data_llama2_2)
    == len(data_llama2_3)
    == len(data_zephyr_1)
...
for i in range(len(data_codellama_3)):
    ...
    if data_codellama_3[i]["achieved_objective"]:
        ...
        print("Codellama did not achieve objective")
...
num_falses_zephyr, num_trues_zephyr, num_responses_zephyr = (
    get_refusal_acceptance_rate_different_percentage_from_file(
        f"results/new_judging_system/{malicious_or_vulnerable}/no_adversarial/after_human_judge_zephyr.json",
...
num_falses_llama2, num_trues_llama2, num_responses_llama2 = (
    get_refusal_acceptance_rate_different_percentage_from_file(
        f"results/new_judging_system/{malicious_or_vulnerable}/no_adversarial/after_human_judge_llama2.json",
        scorers_percentage=[0.5, 0.25, 0.25],
    )
)
...
num_falses_codellama, num_trues_codellama, num_responses_codellama = (
    get_refusal_acceptance_rate_different_percentage_from_file(
        f"results/new_judging_system/{malicious_or_vulnerable}/no_adversarial/after_human_judge_codellama.json",
        scorers_percentage=[0.5, 0.25, 0.25],
    )
)
...
num_falses_llama3_1, num_trues_llama3_1, num_responses_llama3_1 = (
    get_refusal_acceptance_rate_different_percentage_from_file(
        f"results/new_judging_system/{malicious_or_vulnerable}/no_adversarial/after_human_judge_llama3.1.json",
        scorers_percentage=[0.5, 0.25, 0.25],
    )
)
...
num_falses_gemma3_27b, num_trues_gemma3_27b, num_responses_gemma3_27b = (
    get_refusal_acceptance_rate_different_percentage_from_file(
        f"results/new_judging_system/{malicious_or_vulnerable}/no_adversarial/after_human_judge_gemma3:27b.json",
        scorers_percentage=[0.5, 0.25, 0.25],
    )
)
...
num_falses_llama3_1_70b, num_trues_llama3_1_70b, num_responses_llama3_1_70b = (
    get_refusal_acceptance_rate_different_percentage_from_file(
        f"results/new_judging_system/{malicious_or_vulnerable}/no_adversarial/after_human_judge_llama3.1:70b.json",
        scorers_percentage=[0.5, 0.25, 0.25],
    )
)
...
for model_name, list_of_model_datasets in datasets_map.items():
    for individual_dataset_run in list_of_model_datasets:
        ...
        print(
            f"{model_name} run {len(model_acceptance_rates_per_run[model_name])}: {achieved_count}/{num_entries} successful = {acceptance_rate:.3f} acceptance rate"
        )
...
def make_bar_chart_with_std(data, title, baseline_results=None):
    ...
    for model in models:
        print(
            f"{model}: refusal rate: {data[model]['refusal_rate']:.3f} (±{data[model]['refusal_std']:.3f}), "
    ...
    if baseline_results is not None:
        baseline_refusal_rates = [
            baseline_results[model]["refusal_rate"]
            for model in models
        ...
        baseline_refusal_stds = [
            baseline_results[model].get("refusal_std", 0)
            for model in models
            if model in baseline_results
        ]
        baseline_acceptance_stds = [
            baseline_results[model].get("acceptance_std", 0)
            for model in models
            if model in baseline_results
        ]
        ...
        baseline_bars1 = ax.bar(
            baseline_x - 1.5 * width,
            baseline_refusal_rates,
            width,
            yerr=baseline_refusal_stds,
            color="red",
        ...
        baseline_bars2 = ax.bar(
            baseline_x - 0.5 * width,
            baseline_acceptance_rates,
            width,
            yerr=baseline_acceptance_stds,
            color="blue",
...
make_bar_chart_with_std(
    data,
    f"Multi-turn Attack Success/Refusal Rates (3 runs each, {len(data_codellama_1)} entries per run)",
...
if turns_data_for_boxplot:
    ...
    bp = plt.boxplot(
        turns_data_for_boxplot,
        tick_labels=model_labels_for_boxplot,
        patch_artist=True,
        vert=True,
        widths=0.6,
    )
    ...
    plt.title(
        "Distribution of User Turns for Successful Attempts (All Dataset Runs Combined)",
        fontsize=18,
    )
...
for model_name in model_names:
    ...
    if turns_list:
        ...
        print(
            f"{model_name}: Mean Turns = {mean_turns:.2f}, Std Dev = {std_dev_turns:.2f} (from {num_successes_with_turns} successful attempts with turn data)"
        )
    else:
