...
from pyrit.orchestrator.single_turn.flip_attack_orchestrator import (
...
from orchestrator.constants import DEFAULTS, Goals, TargetModel, TypesOfAttacks
...
async def over_refusal_test(
    ...
    **kwargs,
    ...
    seed = args["seed"]
    args["temperature_judges"]
    args["temperature_target"]
    ...
    try:
        ...
        jury_1 = OllamaChatTarget(
            model_name=jury_models[0],
            endpoint=ollama_host + "/api/chat",
            timeout=None,
            options={"num_predict": 1000, "seed": seed},
        )  #'temperature': temperature_judges})
        jury_2 = OllamaChatTarget(
            model_name=jury_models[1],
            endpoint=ollama_host + "/api/chat",
            timeout=10000,
            options={"num_predict": 1000, "seed": seed},
        )  #'temperature': temperature_judges})
        jury_3 = OllamaChatTarget(
            model_name=jury_models[2],
            endpoint=ollama_host + "/api/chat",
            timeout=10000,
            options={"num_predict": 1000, "seed": seed},
        )  #'temperature': temperature_judges})
        ...
        refusal_scorer_jury_1 = SelfAskRefusalScorer(
            ...
            system_prompt_path=pathlib.Path(
                "./pyrit/datasets/score/refusal/refusal_to_generate_code.yaml"
            ),
        )
        ...
        prompt_sending_orchestrator = PromptSendingOrchestrator(
            target,
            scorers=[
                refusal_scorer_jury_1,
                refusal_scorer_jury_2,
                refusal_scorer_jury_3,
            ],
            verbose=True,
            batch_size=3,
        )
        ...
        await memory.add_seed_prompts_to_memory_async(
            prompts=or_bench_dataset.prompts, added_by="jd"
        )  # type: ignore
        ...
        await prompt_sending_orchestrator.send_normalizer_requests_async(
        ...
        result_json = prompt_sending_orchestrator.output_conversations_to_json(
            file_path="debug_results/orr_atks_test.json"
        )
        ...
        store_run(
            ...
            attack_type=TypesOfAttacks.OVER_REFUSAL_ATTACK.value,
        )
...
async def launch_attack_template(ollama_host, **kwargs):
    ...
    try:
        ...
        path = (
            ...
            if template_path
            else pathlib.Path("datasets/JailBreakV_28K_clean.yaml")
        )
                ...
                for item in data:
                    if "Prompt" in item:
                        formatted_prompts.append(
                            {"value": item["Prompt"], "data_type": "text"}
                        )
                    elif "value" in item:
        ...
        jury_1 = OllamaChatTarget(
            ...
            endpoint=ollama_host + "/api/chat",
            timeout=None,
            options={"num_predict": 5000, "seed": seed},
        )  #'temperature': temperature_judges})
        jury_2 = OllamaChatTarget(
            model_name=jury_models[1],
            endpoint=ollama_host + "/api/chat",
        ...
        result_json = prompt_sending_orchestrator.output_conversations_to_json(
            file_path="debug_results/template_atks_test.json"
        )
        # score responses
        ...
        metrics = get_run_metrics(
            result_json, atk_type="attack_template", n_goals=len(goals_list)
        )
...
async def launch_crescendo_attack(ollama_host, **kwargs):
    ...
    try:
        ...
        results = await orchestrator.run_attacks_async(
            objectives=goals_list, batch_size=1
        )  # type: ignore
        ...
        orchestrator.output_conversations_to_json(
            file_path="debug_results/crescendo_atk_test.json"
        )
        ...
        static_metric = (
            vuln_analysis(data) if label == Goals.VULNERABLE_GOALS.value else None
        )
...
async def launch_flip_attack(ollama_host, **kwargs):
    ...
    try:
        ...
        OllamaChatTarget(
            ...
            endpoint=ollama_host + "/api/chat",
            timeout=5000,
            options={"seed": seed, "temperature": temperature_attacker},
        )
        judge = OllamaChatTarget(
            model_name=judge_model_name,
            endpoint=ollama_host + "/api/chat",
            timeout=5000,
            options={"seed": seed, "temperature": temperature_judge},
        )
        ...
        orchestrator = FlipAttackOrchestrator(
            objective_target=objective_target,
            scorers=[refusal_scorer_judge],
            verbose=True,
            batch_size=1,
        )

        await orchestrator.send_prompts_async(
            prompt_list=goals_list, metadata=goals_list
        )  # type: ignore

        orchestrator.output_conversations_to_json(
            file_path="results/attacker_llm/malicious/single_turn/role_play/video_game2/gemma3_27b_3.json"
        )
...
async def launch_role_play_attack(ollama_host, **kwargs):
    ...
    try:
        ...
        attacker = OllamaChatTarget(
            ...
            timeout=2000,
            options={"seed": seed},
        )  #'temperature': temperature_attacker})
        jury_1 = OllamaChatTarget(
            model_name=jury_models[0],
            endpoint=ollama_host + "/api/chat",
            timeout=None,
        ...
        orchestrator = RolePlayOrchestrator(
            objective_target=objective_target,
            adversarial_chat=attacker,
            role_play_definition_path=role_play_option,
            scorers=[
                refusal_scorer_jury_1,
                refusal_scorer_jury_2,
                refusal_scorer_jury_3,
        ...
        print("results:")
        await orchestrator.send_as_normalizer_requests_async(goals_list=goals_list)  # type: ignore
        result_json = orchestrator.output_conversations_to_json(
            file_path="debug_results/role_play_atk_test.json"
        )
