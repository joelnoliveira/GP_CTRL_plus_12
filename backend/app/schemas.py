...
class AttackRequest(BaseModel):
    ...
    temperature_judges: float = Field(
        default=0.1, ge=0.0, le=2.0, description="Temperature for judge models"
    )
    temperature_attacker: float = Field(
        default=0.1, ge=0.0, le=2.0, description="Temperature for attacker models"
    )
    temperature_target: float = Field(
        default=0.1, ge=0.0, le=2.0, description="Temperature for target model"
    )
    ...
    target_model_name: str = Field(
        default="qwen2.5:1.5b", description="Target model to attack"
    )
    attacker_model_name: str = Field(
        default="qwen2.5:1.5b", description="Model that generates attacks"
    )
    judge_model_name: str = Field(
        default="qwen2.5:1.5b", description="Judge model (for Crescendo/Flip)"
    )
    ...
    role_play_option_id: Optional[int] = Field(
        default=2, description="Role play option identifier (only for ROLE_PLAY_ATTACK)"
    )
    target_provider: Optional[str] = Field(
        default="OLLAMA",
        description="Model provider for the target model (if required)",
    )
    api_key: Optional[str] = Field(
        default=None, description="API key for the target model provider (if required)"
    )
...
class AttackTemplateRequest(BaseModel):
    ...
    template_dataset_id: Optional[int] = Field(
        default=None, description="Template dataset identifier (optional)"
    )
    target_provider: Optional[str] = Field(
        default="OLLAMA",
        description="Model provider for the target model (if required)",
...
class OverRefusalTestRequest(BaseModel):
    ...
    target_model_name: str = Field(
        default="qwen2.5:1.5b", description="Target model to test"
    )
    jury_models: list[str] = Field(
        default=["qwen2.5:1.5b", "qwen2.5:1.5b", "qwen2.5:1.5b"],
        description="List of 3 jury models",
