...
\begin{itemize}
  ...
  \begin{itemize}
    \item Dependência do servidor externo do professor (Ollama) para a execução dos modelos de LLM, cuja disponibilidade e desempenho não estão sob controlo direto.
    \item Limitações de largura de banda e latência de rede ao comunicar com o servidor externo.
    \item Dependência de ferramentas externas (ex: PyRIT) que podem impor limites de desempenho ou disponibilidade.
  \end{itemize}
...
\begin{itemize}
  \item Recursos Computacionais: Disponibilidade contínua do servidor externo (Ollama) fornecido pelo professor para execução dos modelos de LLM.
