...
def create_matrix_visualization(results, prompt_to_id, category_filter=None):
    ...
    if category_filter:
        filtered_results = {
            k: v for k, v in results.items() if k.startswith(category_filter)
        }
    else:
    ...
    for result_key, data in filtered_results.items():
        ...
        clean_name = (
            result_key.replace(f"{category_filter}_", "")
            if category_filter
            else result_key
        )
    ...
    objective_ids = [
        prompt_to_id[prompt] for prompt in category_prompts if prompt in prompt_to_id
    ]
...
def create_heatmap(df, title="Model Evaluation Results", filename_suffix=""):
    ...
    sns.heatmap(df, annot=False, cmap=cmap, cbar=False, linewidths=0.5, vmin=0, vmax=1)
...
def create_summary_stats(df, id_to_prompt):
    ...
    for i, (objective_id, rate) in enumerate(prompt_success.head(10).items()):
        if not np.isnan(rate):
            prompt_preview = (
                id_to_prompt[objective_id][:50] + "..."
                if len(id_to_prompt[objective_id]) > 50
                else id_to_prompt[objective_id]
            )
...
print(
    "✅ Objective mapping saved as 'objective_id_mapping.csv' and 'objective_id_mapping.json'"
)
...
def create_heatmap_with_dataset_sections(
    df, title="Model Evaluation Results", filename_suffix=""
):
    ...
    # Create the heatmap
    sns.heatmap(
        df, annot=False, cmap=cmap, cbar=False, linewidths=0.5, vmin=0, vmax=1, ax=ax
    )
    ...
    for i in range(len(df.columns)):
        if (
            (i + 1) % models_per_dataset != 0
        ):  # Don't add line after the last model in each dataset
...
if not results:
    ...
    df_category = create_matrix_visualization_by_dataset(
        results, prompt_to_id, category
    )
    ...
    if df_category.empty:
        ...
        title = f"{category.capitalize()} Code Generation - Per Goal Results"
        plt_category = create_heatmap_with_dataset_sections(
            df_category, title, f"_{category}"
        )
