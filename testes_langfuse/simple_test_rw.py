...
def test_simple_rw():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(
                "⚠️ Trace not found (404). It might take longer to ingest or the endpoint is different."
            )
            # Fallback check for health
            print(
                f"Checking health: {requests.get(f'{LANGFUSE_HOST}/api/public/health').status_code}"
            )
            ...
            response = requests.get(
                url, auth=(LANGFUSE_PUBLIC_KEY, LANGFUSE_SECRET_KEY)
            )
            if response.status_code == 200:
