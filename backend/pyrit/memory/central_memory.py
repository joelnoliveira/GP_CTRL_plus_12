...
class CentralMemory:
    ...
    def set_memory_instance(cls, passed_memory: MemoryInterface) -> None:
        ...
        logger.info(
            f"Central memory instance set to: {type(cls._memory_instance).__name__}"
        )
    ...
    def get_memory_instance(cls) -> MemoryInterface:
        ...
        if cls._memory_instance:
            logger.info(
                f"Using existing memory instance: {type(cls._memory_instance).__name__}"
            )
            return cls._memory_instance
        else:
            raise ValueError(
                "Central memory instance has not been set. Use `set_memory_instance` to set it."
            )
